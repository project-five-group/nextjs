...
const parseHex = (rawHex: string): TStringHex => {
  ...
  const twoDigitHexR = isShort
    ? `${rawHex.slice(0, 1)}${rawHex.slice(0, 1)}`
    : rawHex.slice(0, 2);
  const twoDigitHexG = isShort
    ? `${rawHex.slice(1, 2)}${rawHex.slice(1, 2)}`
    : rawHex.slice(2, 4);
  const twoDigitHexB = isShort
    ? `${rawHex.slice(2, 3)}${rawHex.slice(2, 3)}`
    : rawHex.slice(4, 6);
  const twoDigitHexA =
    (isShort
      ? `${rawHex.slice(3, 4)}${rawHex.slice(3, 4)}`
      : rawHex.slice(6, 8)) || 'ff';
...
const formatRgb = ({ r, g, b, a: parsedA }: TDecimalHex, a?: number) =>
  `rgba(${r}, ${g}, ${b}, ${a || parsedA})`;
